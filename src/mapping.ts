...
import { BLOCK_SIDEVIEWS_ACTIVATED, GOTCHI_TYPE } from "./constants";
...
export function handleUpdateSvg(event: UpdateSvg): void {
    let typesAndSizes = event.params._typesAndIdsAndSizes;
    for (let i = 0; i < typesAndSizes.length; i++) {
        let entry = typesAndSizes[i];
        let type = entry.svgType;
        let ids = entry.ids;
        if (type == GOTCHI_TYPE) {
            for (let j = 0; j < ids.length; j++) {
                let id = ids[j];
                let gotchi = updateSideViews(id);
                if (gotchi != null) {
                    gotchi.save();
                }
            }
